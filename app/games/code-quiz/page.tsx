...
import { RatRaceGame } from "@/components/rat-race-game"
...
export default function CodeQuizPage() {
  ...
  const [ratAction, setRatAction] = useState<"running" | "jumping" | "falling">("running")
  ...
  // Update distance while running
  useEffect(() => {
    if (ratAction !== "running") return
  ...
  const handleAnswer = (answerIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setRatAction("jumping")
      setScore((prev) => prev + 1)
      setDistance((prev) => prev + 20) // Bonus distance for correct answer
    } else {
      setGameState("wrong")
      setRatAction("falling")
    }
    ...
    return (
      <div className="min-h-screen bg-background code-pattern flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 bg-card/90 backdrop-blur-sm border-2 border-primary/40 text-center space-y-6">
          ...
          <h2 className="text-3xl font-bold text-foreground">Race Complete!</h2>
          <div className="space-y-4">
            ...
            <div>
              <p className="text-3xl font-bold text-secondary">{distance}m</p>
              <p className="text-muted-foreground">total distance covered</p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-background code-pattern">
      ...
      <div className="flex items-center justify-between p-4 bg-card/50 backdrop-blur-sm border-b border-primary/20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm font-mono">
            ...
            <span className="text-secondary font-bold">{distance}m</span>
          </div>
          <div className="text-sm font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 h-[calc(100vh-80px)]">
          ...
          <div className="max-w-xl w-full space-y-6">
            ...
            {gameState !== "playing" && (
              <div className="text-center animate-slide-in">
                {gameState === "correct" && (
                  <p className="text-lg font-bold text-primary">Correct! Rat jumped over the pothole!</p>
                )}
                {gameState === "wrong" && (
                  <p className="text-lg font-bold text-destructive">Wrong answer! Rat fell into the pothole!</p>
                )}
                {gameState === "timeout" && (
                  <p className="text-lg font-bold text-destructive">Time's up! Rat fell into the pothole!</p>
                )}
              </div>
            )}
          </div>
        ...
        {/* Right Side - Rat Race Animation */}
        <div className="flex items-center justify-center p-8 bg-gradient-to-br from-secondary/5 to-accent/5">
          <RatRaceGame action={ratAction} distance={distance} />
        </div>
      </div>
    </div>
  )
